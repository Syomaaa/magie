...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local mercure1 = ents.Create( "mercure1" )
			mercure1:SetPos( pos )
			mercure1:SetAngles( ang ) 
			mercure1:SetOwner( own )
			mercure1:Spawn() 
			mercur1:Activate() 
			own:DeleteOnRemove( mercure1 )
			mercure1:GetPhysicsObject():SetVelocity( dir )
			mercure1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
            ...
            self:EmitSound( "ambient/levels/launch/debris02.wav" , 100, 100,0.4)
			local phys = self:GetPhysicsObject()
			if phys:IsValid() then
				phys:Wake()
				phys:EnableGravity(false)
			end
			local idx = "mercure1111"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[24]_yushiro_slash_main", 3, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "mercure1" )
end