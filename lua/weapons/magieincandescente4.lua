SWEP.PrintName = "Incandescente 4"
SWEP.Slot = 0
SWEP.SlotPos = 1
SWEP.DrawAmmo = false
...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		if SERVER then
			...
			local entnezu4 = ents.Create("nezu4")
			entnezu4:SetPos(pos)
			entnezu4.Owner = self.Owner
			entnezu4:Spawn()
			SafeRemoveEntityDelayed(entnezu4,1)

			local own = self.Owner
		
			timer.Create("dmgnezu4"..self.Owner:EntIndex(),config.interval,config.nb,function()
		...
		local idx = "nezu4"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[21]_nezuko_ground", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	if CLIENT then
		function ENT:Draw()
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "[21]_nezuko_ground", ef )
				self:DrawShadow( false )
			end
		end
	end
	scripted_ents.Register( ENT, "nezu4" )
end