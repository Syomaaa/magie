...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if tra.HitWorld then
			local ciel_lune = ents.Create( "ciel_lune" ) 
			ciel_lune:SetAngles(  Angle( ang.pitch, ang.yaw, 0 ) ) 
			ciel_lune:SetPos( pos + Vector(0,0,1000) ) 
			ciel_lune.Owner = own
			ciel_lune:Spawn() 
			ciel_lune:Activate() 
			own:DeleteOnRemove( ciel_lune )
			ciel_lune:EmitSound( "ambient/levels/canals/tunnel_wind_loop1.wav",400 )

			timer.Simple(config.tmp,function()
				ciel_lune:StopSound( "ambient/levels/canals/tunnel_wind_loop1.wav",400 )
			end)
		end
...
if true then
	...
	function ENT:Initialize()
		...
		self:SetModel( "models/planets/luna_big.mdl" )
	...
	if CLIENT then
		function ENT:Draw()
			...
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "lune_big_effect", ef )
				self:DrawShadow( false )
			end
		end
	end
	
	scripted_ents.Register( ENT, "lune_big" )
end
...
if true then
	...
	function ENT:Initialize()
		...
		self.Shake:SetKeyValue( "amplitude", "6" )
		...
		timer.Simple(config.tmpFall,function()
			if IsValid(self) then
				local lune = ents.Create( "lune_big" ) 
				lune:SetPos( self:GetPos() ) 
				lune:Spawn() 
				lune:Activate() 
			end
		end)
	...
	if CLIENT then
		function ENT:Draw()
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "ciel_lune_effect", ef )
				self:DrawShadow( false )
			end
		end
	end
	scripted_ents.Register( ENT, "ciel_lune" )
end
...
if true then
	...
	effects.Register( EFFECT, "lune_big_effect" )
end
...
if true then
	...
	effects.Register( EFFECT, "ciel_lune_effect" )
end