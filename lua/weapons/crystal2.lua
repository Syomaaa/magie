local JY = {}
JY.Name             = "Crystal 2"       // Nom de l'arme
JY.Category         = "Crystal"         // nom de la catégorie

JY.Debug            = false              // Pour t'aidé a config la zone

JY.MaxDistance      = 2500                // La distance max pour l'attaque. Met -1 pour aucune limite
JY.RayonDetection   = 125                // Le rayon
JY.Cooldown         = 11.5                // Cooldown en seconde
JY.Duration         = 1.5                 // Durée de l'attaque en seconde
JY.Damage           = 200               // Dégat
JY.Model            = "models/props_abandoned/crystals_fixed/crystal_default/crystal_cluster_small_a.mdl"   // Le modèle
...
local function shake(ent, time)
    if !IsValid(ent) then return end
    timer.Simple(time, function()
        if !IsValid(ent) then return end
        ent:SetAngles(Angle(0 + (math.sin(CurTime()) + math.random()), 0 + (math.sin(CurTime()) + math.random()), 0))
        shake(ent, time)
    end)
end
...
function SWEP:PrimaryAttack()
    ...
    if (self:GetOwner()["JY_CD_".. JY.Name] || 0) > CurTime() then return end
    ...
    timer.Simple(JY.Duration * .5, function()
