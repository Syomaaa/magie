...
function SWEP:PrimaryAttack()
	...
	local own = self.Owner
		...
		local zone_eff = ents.Create( "prop_physics" )
		zone_eff:DrawShadow( false )
		zone_eff:SetModel( "models/effects/portalrift.mdl" )
		zone_eff:PhysicsInit( SOLID_NONE )
		zone_eff:SetMoveType( MOVETYPE_NONE )
		zone_eff:SetRenderMode(5)
		zone_eff:SetPos( own:GetPos() + Vector(0,0,1000) )
		zone_eff:SetModelScale(2,1)
		zone_eff:Spawn()
		SafeRemoveEntityDelayed(zone_eff,5)

		local eto = ents.Create( "etoile3" )
		eto:SetPos( own:GetPos() )
		eto:SetOwner( own )
		eto:Spawn() 
		eto:Activate() 
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			ParticleEffectAttach( "[0]_starsphere_ambient", 1, self, 1 )
	...
	function ENT:Think()
		...
		local dir = self:GetAngles():Up()*-100 +VectorRand():GetNormal()*5

		local star = ents.Create( "etoile3_star" )
		star:SetPos( pos )
		star:SetOwner( own )
		star:Spawn() 
		star:Activate() 
		own:DeleteOnRemove( star )
		star:GetPhysicsObject():SetVelocity( dir )
		star:SetPhysicsAttacker( own )
	...
	scripted_ents.Register( ENT, "etoile3" )
end
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "etoile3"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					ParticleEffectAttach( "[1]_light_trail", 1, self ,1)
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "etoile3_star" )
end