...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local foudrenoir1 = ents.Create( "foudrenoir1" )
			foudrenoir1:SetPos( pos )
			foudrenoir1:SetAngles( ang ) 
			foudrenoir1:SetOwner( own )
			foudrenoir1:Spawn() 
			foudrenoir1:Activate() 
			own:DeleteOnRemove( foudrenoir1 )
			foudrenoir1:GetPhysicsObject():SetVelocity( dir )
			foudrenoir1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "fffoudrreeenoir2"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[5]_darklightning_projectile_add_3", 3, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "foudrenoir1" )
end