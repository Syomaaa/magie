...
if true then
	...
	function ENT:Initialize()
		self:DrawShadow( false ) if !SERVER then return end
		self:SetModel( "models/models/bcprops/supportspade_2.mdl" )
	...
	if CLIENT then
		function ENT:Draw()
			...
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ex = EffectData()
				ex:SetOrigin(self:GetPos() +Vector(0,0,100))
				util.Effect("wall",ex) 
			end
		end
	end
	scripted_ents.Register( ENT, "wallphysic" )
end
...
if true then
	...
	effects.Register( EFFECT, "powerup_effect" )
end