...
SWEP.PrintName = "Transformation 4"
...
SWEP.TransformationModel = "models/echo/ark/argentavis_pm.mdl"
SWEP.TransformationSpeedBoost = 2.5
SWEP.TransformationJumpBoost = 1.5
SWEP.WeaponToGive = "transfo4arme"
...
local function AdjustModelScale(player, targetScale, duration, onComplete)
    ...
    timer.Create("AdjustModelScaleTimer4_" .. player:EntIndex(), 0.1, duration / 0.1, function()
...
function SWEP:CancelTransformation(owner)
    ...
    owner:SetModelScale(owner.PreviousModelScale or 1, 0)
...
hook.Add("PlayerShouldTakeDamage", "PreventFallDamageAfterTransformation4", function(player, attacker)
    if IsValid(player) and player:IsPlayer() then
        local weapon = player:GetActiveWeapon()
        if IsValid(weapon) and (weapon:GetClass() == "weapon_transformation1" or weapon:GetClass() == "weapon_transformation2" or weapon:GetClass() == "weapon_transformation3" or weapon:GetClass() == "weapon_transformation4") and weapon.IsJumpingAfterTransformation then
            return false
        end
    end
end)

hook.Add("PlayerDeath", "ResetPlayerModelOnDeath4", function(player)
    if IsValid(player) and player.PreviousModel then
        ...
        player:SetJumpPower(player.PreviousJumpPower)
