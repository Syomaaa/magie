...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
			...
			local file2 = ents.Create( "file2" )
			file2:SetOwner( own ) 
			file2:SetPos( pos )
			file2:SetAngles( Angle( 0, own:GetAngles().yaw, 0 ) ) 
			file2:Spawn() 
			file2:Activate()
...
if true then
		...
		if SERVER then
			...
			timer.Create(idx,0.2,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[13]_spider_bloom", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "file2" )
end