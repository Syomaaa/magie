...
local config = {}
config.dmg1 = 90  
config.dmg2 = 110 --dgt de base entre .. et ..

config.dmgup1 = 125
config.dmgup2 = 150 --dgt click droit saut entre .. et ..

config.dmgleap1 = 125
config.dmgleap2 = 150 --dgt click droit en avant entre .. et ..

config.dmgslh1 = 200
config.dmgslh2 = 200 --dgt slash entre .. et ..
...
SWEP.Cooldown = 10 --cooldown general pour utiliser une atk spe
SWEP.Cooldown1 = 20 --cooldown shield
SWEP.Cooldown2 = 10 --cooldown slash
...
function SWEP:SetupDataTables()
	self:NetworkVar( "Int", 0, "AntiMagie2" )
	if (SERVER) then
		self:SetAntiMagie2( 1 )
	end
end
...
function SWEP:Reload()
	...
	elseif self:GetAntiMagie2() == 2 then
		if self.NextAction > CurTime() then return end
		if self.CooldownDelay < CurTime() then
			self:SlashAttack()
...
function SWEP:BlockAttack()
	...
	if self.CooldownDelay1 < CurTime() then if !SERVER then return end
		...
		local shield = ents.Create( "shield1" ) 
			...
			if(IsValid(self) && self:GetOwner():Alive()) then
				...
				timer.Simple(0.15, function()
					if(IsValid(self) && self:GetOwner():Alive()) then
						...
						local slash1 = ents.Create( "slash" )
						slash1:SetPos( pos )
						slash1:SetAngles( ang ) slash1:SetOwner( own )
						slash1:Spawn() slash1:Activate() own:DeleteOnRemove( slash1)
						slash1:GetPhysicsObject():EnableGravity( false )
						slash1:GetPhysicsObject():SetVelocity( dir )
						slash1:SetPhysicsAttacker( own )
					end
				end)
			end
		end)
...
function SWEP:DrawHUD()
	...
	local iconslot = 1
	local SelectedPower = self:GetAntiMagie2()
	local IconCaseColor = Color(20,20,20, 150 )
	
	
	for id, t in pairs( AntiMagie2 ) do
...
hook.Add( "ShouldDrawLocalPlayer", "AntiMagieThirdPDraw3", function (ply)
	if ply:GetActiveWeapon():IsValid() then
		if ply:IsPlayer() && ply:Alive() && ply:GetActiveWeapon():GetClass() == "antimagie3" then
			return true
		end
	end
end)

hook.Add("PlayerButtonDown", "AntiMagie2binds", function( ply, button )
	if config.canSwitch == true then if !SERVER then return end
	if button == ply:GetInfoNum( "TouchesBind1", 15) and ply:IsValid() then
		if ply:GetActiveWeapon():IsValid() then
			if ply:GetActiveWeapon():GetClass() == "antimagie3" then
					ply:GetActiveWeapon():SetAntiMagie2(1)
			end
		end
	elseif button == ply:GetInfoNum( "TouchesBind2", 28) and ply:IsValid() then
		if ply:GetActiveWeapon():IsValid() then
			if ply:GetActiveWeapon():GetClass() == "antimagie3" then
					ply:GetActiveWeapon():SetAntiMagie2(2)
			end
		end
	end
	end
	...
	if CLIENT then
		function ENT:Draw() self:DrawModel()
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "slash_effect", ef )
				self:DrawShadow( false )
			end
		end
	end
	scripted_ents.Register( ENT, "slash" )
end

if true then
	local ENT = {}
	ENT.Base = "base_anim"
	ENT.PrintName = "shield1"
	ENT.Spawnable = false
	ENT.RenderGroup = RENDERGROUP_BOTH
	...
	function ENT:Initialize()
		...
		self:SetAngles(Angle(0,own:EyeAngles().y + 135,0))
		self:SetMaterial( "models/shadertest/shader4" ) 
	...
	if CLIENT then
		function ENT:Draw()
			...
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "shield1_effect", ef )
				self:DrawShadow( false )
			end
		end
	end
	scripted_ents.Register( ENT, "shield1" )
end
...
if true then
	...
	effects.Register( EFFECT, "slash_effect" )
end
...
if true then
	...
	function EFFECT:Think() local ent = self.Owner
		if IsValid( ent ) then self:SetRenderBoundsWS( ent:GetPos() + ent:OBBMaxs(), ent:GetPos() + ent:OBBMins() )
			...
			if self.NextEmit < CurTime() then self.NextEmit = CurTime() + 0.01
				for i=1, 3 do
					local particle = self.Emitter:Add( "particles/smokey", ent:WorldSpaceCenter() + Vector(math.random(-60,60),math.random(-60,60),math.random(-50,50)) )
					if particle then  local size = math.Rand(2,3 )
						particle:SetVelocity( VectorRand( -1, 1 ):GetNormal() * 50 )
						particle:SetLifeTime( 0 )
	...
	effects.Register( EFFECT, "shield1_effect" )
end