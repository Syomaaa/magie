...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local corruption1 = ents.Create( "corruption1" )
			corruption1:SetPos( pos )
			corruption1:SetAngles( ang ) 
			corruption1:SetOwner( own )
			corruption1:Spawn() 
			corruption1:Activate() 
			own:DeleteOnRemove( corruption1 )
			corruption1:GetPhysicsObject():SetVelocity( dir )
			corruption1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "corruption1"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[2]_spear_projectile", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "corruption1" )
end