...
SWEP.PrintName 		      = "Incandescente 2" 
SWEP.Author 		      = "Brounix" 
SWEP.Instructions 	      = "" 
SWEP.Contact 		      = "" 
...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
			...
			local nezu2 = ents.Create( "nezu2" )
			nezu2:SetOwner( own ) 
			nezu2:SetPos( pos )
			nezu2:SetAngles( Angle( 0, own:GetAngles().yaw, 0 ) ) 
			nezu2:Spawn() 
			nezu2:Activate()
...
if true then
		...
		if SERVER then
			...
			timer.Create(idx,0.2,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[21]_nezuko_changeform", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "nezu2" )
end