...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local neige1 = ents.Create( "neige1" )
			neige1:SetPos( pos )
			neige1:SetAngles( ang ) 
			neige1:SetOwner( own )
			neige1:Spawn() 
			neige1:Activate() 
			own:DeleteOnRemove( neige1 )
			neige1:GetPhysicsObject():SetVelocity( dir )
			neige1:SetPhysicsAttacker( own )
		end
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "neige1"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "snow_ball", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "neige1" )
end