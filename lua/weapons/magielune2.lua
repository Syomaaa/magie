...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
			...
			local lune2 = ents.Create( "lune2" )
			lune2:SetOwner( own ) 
		    lune2:SetPos( pos )
			lune2:SetAngles( Angle( 0, own:GetAngles().yaw, 0 ) ) 
			lune2:Spawn() 
			lune2:Activate()
...
if true then
		...
		if SERVER then
			...
			timer.Create(idx,0.2,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[7]_moontrap_add_2", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "lune2" )
end