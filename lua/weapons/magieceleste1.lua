...
config.addVie = 10 -- vie en plus par touché
...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local celeste1 = ents.Create( "celeste1" )
			celeste1:SetPos( pos )
			celeste1:SetAngles( ang ) 
			celeste1:SetOwner( own )
			celeste1:Spawn() 
			celeste1:Activate() 
			own:DeleteOnRemove( celeste1 )
			celeste1:GetPhysicsObject():SetVelocity( dir )
			celeste1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "celeste1"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "cargdigger_mont_nizigo", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
			...
			for k,v in pairs(ents.FindInSphere(self:GetPos() ,config.zone)) do
                if IsValid(v) and v != own and (v:IsPlayer() or v:IsNPC() or type( v ) == "NextBot") then
					...
					v:TakeDamageInfo(dmginfo)
					  local pushDirection = (v:GetPos() - self:GetPos()):GetNormalized()
						v:SetVelocity(pushDirection * -150) -- Ajuste la force de recul ici (500 par défaut)
	...
	scripted_ents.Register( ENT, "celeste1" )
end