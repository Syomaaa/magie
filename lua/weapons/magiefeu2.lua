...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
			...
			local feu2 = ents.Create( "feu2" )
			feu2:SetOwner( own ) 
			feu2:SetPos( pos )
			feu2:SetAngles( Angle( 0, own:GetAngles().yaw, 0 ) ) 
			feu2:Spawn() 
			feu2:Activate()
...
if true then
		...
		if SERVER then
			...
			timer.Create(idx,0.2,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[10]_fire_blast", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "feu2" )
end