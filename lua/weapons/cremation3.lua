...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		local magiecremation3 = ents.Create("magiecremation3")
		magiecremation3:SetPos(self.Owner:GetPos())
		magiecremation3:SetOwner(self.Owner )
		magiecremation3:Spawn()
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "magiecremation3"..self:EntIndex()
			timer.Create(idx,0.01,1,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[5]_ash_dash", 1, self, 1 )
				end
			end)
	...
	function ENT:Think()
		...
		if self:GetPos():Distance( ptt ) > 65 then 
			self:SetPos( self:GetPos() + ( ptt - self:GetPos() ):GetNormal()*65 ) 
		end
	...
	scripted_ents.Register( ENT, "magiecremation3" )
end