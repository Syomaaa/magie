...
config.addVie = 20 -- vie en plus par touché
...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local dent = ents.Create( "sang" )
			dent:SetPos( pos )
			dent:SetAngles( ang ) 
			dent:SetOwner( own )
			dent:Spawn() 
			dent:Activate() 
			own:DeleteOnRemove( dent )
			dent:GetPhysicsObject():SetVelocity( dir )
			dent:SetPhysicsAttacker( own )
			if self.Owner:Health()-config.delVie<=0 then
				self.Owner:Kill()
			end
			dent:EmitSound("physics/cardboard/cardboard_box_impact_bullet1.wav", 65, math.Rand(30,60), 0.8)
	...
	if CLIENT then
		function ENT:Draw()
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "sang_effect", ef )
				self:DrawShadow( false )
			end
		end
	end
	scripted_ents.Register( ENT, "sang" )
end

if SERVER then return end

if true then
	local Mat2 = Material( "sang/dent" )
	local EFFECT = {}
		...
		if IsValid( ent ) then self:SetRenderBoundsWS( ent:GetPos() + ent:OBBMaxs(), ent:GetPos() + ent:OBBMins() )
			...
			if self.NextEmit < CurTime() then self.NextEmit = CurTime() + 0.01
				for i=1, 3 do
					...
					if particle then  local size = math.Rand( 3, 5 )  local tmp = math.Rand( 0.2, 0.5)
						particle:SetVelocity( VectorRand( -1, 1 ):GetNormal() * 50 )
						particle:SetLifeTime( 0 )
						particle:SetDieTime( tmp )
						...
						SafeRemoveEntityDelayed(particle,tmp)
					end
				end
			end
			return true
	...
	function EFFECT:Render()
		local ent = self.Owner  if IsValid( ent ) then local siz = ( math.abs( math.sin( CurTime()*5 ) )*15 +20 )
			render.SetMaterial( Mat2 ) render.DrawSprite( ent:WorldSpaceCenter(), siz, siz, Color( 255, 255, 255 ) )
		end
	end
	effects.Register( EFFECT, "sang_effect" )
end