...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local saison1 = ents.Create( "saison1" )
			saison1:SetPos( pos )
			saison1:SetAngles( ang ) 
			saison1:SetOwner( own )
			saison1:Spawn() 
			saison1:Activate() 
			own:DeleteOnRemove( saison1 )
			saison1:GetPhysicsObject():SetVelocity( dir )
			saison1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "ssaisson1"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "plants_add", 3, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "saison1" )
end