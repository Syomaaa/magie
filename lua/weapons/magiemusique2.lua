...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
			...
			local musique2 = ents.Create( "musique2" )
			musique2:SetOwner( own ) 
			musique2:SetPos( pos )
			musique2:SetAngles( Angle( 0, own:GetAngles().yaw, 0 ) ) 
			musique2:Spawn() 
			musique2:Activate()
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "musiqueeeee2"..self:EntIndex()
			timer.Create(idx,0.2,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[1]_sound_sphere", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "musique2" )
end