...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		if SERVER then
			...
			local entsaison4 = ents.Create("saison4")
			entsaison4:SetPos(pos)
			entsaison4.Owner = self.Owner
			entsaison4:Spawn()
			SafeRemoveEntityDelayed(entsaison4,1)

			local own = self.Owner
		
			timer.Create("dmgsaison4"..self.Owner:EntIndex(),config.interval,config.nb,function()
...
if true then
	...
	function ENT:Initialize()
		...
		local idx = "sssaaaiiisson4"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[10]_rengoku_tornado", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "saison4" )
end