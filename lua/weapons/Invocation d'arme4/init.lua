...
local config = {}
config.dmg1 = 150 
config.dmg2 = 175 --dgt de base entre .. et ..
...
config.magiedmg1 = 500
config.magiedmg2 = 500
config.nb = 15 --nn de papillon
config.zone = 300
SWEP.Cooldown2 = 20
...
function SWEP:SecondaryAttack()
	...
	if self.CooldownDelay2 < CurTime() then if !SERVER then return end
		...
		for i=1,config.nb do
			local magieberserker4 = ents.Create("magieberserker4")
			magieberserker4:SetPos(pos)
			magieberserker4.Owner = self.Owner
			magieberserker4:Spawn()
		end

		timer.Simple(0.4,function()
			for k,v in pairs(ents.FindInSphere(self.Owner:GetPos() +  Angle(0, self.Owner:EyeAngles().y, 0):Forward() * 300 ,config.zone)) do
				if IsValid(v) and v != self.Owner and (v:IsPlayer() or v:IsNPC() or type( v ) == "NextBot") then
					local dmginfo = DamageInfo()
					dmginfo:SetDamageType( DMG_GENERIC  )
					dmginfo:SetDamage( math.random(config.magiedmg1,config.magiedmg1) )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "magieberserker4"..self:EntIndex()
			timer.Create(idx,0.2,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[7]_pushwave", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "magieberserker4" )
end
