...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local feerique1 = ents.Create( "feerique1" )
			feerique1:SetPos( pos )
			feerique1:SetAngles( ang ) 
			feerique1:SetOwner( own )
			feerique1:Spawn() 
			feerique1:Activate() 
			own:DeleteOnRemove( feerique1 )
			feerique1:GetPhysicsObject():SetVelocity( dir )
			feerique1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "feerique1"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[11]_insect_slash", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "feerique1" )
end