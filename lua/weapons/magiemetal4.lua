...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if tra.HitWorld then
			local metal_lune = ents.Create( "metal_lune" ) 
			metal_lune:SetAngles(  Angle( 90, 0, 0 ) ) 
			metal_lune:SetPos( pos + Vector(0,0,1000) ) 
			metal_lune.Owner = own
			metal_lune:Spawn() 
			metal_lune:Activate() 
			own:DeleteOnRemove( metal_lune )
		end
...
if true then
	...
	function ENT:Initialize()
		...
		self:SetModel( "models/kuma96/ludwigholyblade/lhbgreatsword.mdl" )
	...
	function ENT:Think()
		...
		if SERVER then
			...
			elseif self:GetPos().z <= config.hitpos.z then
				self:EmitSound("doors/heavy_metal_stop1.wav",100,100,3)
	...
	if CLIENT then
		function ENT:Draw()
			...
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "metal_big_effect", ef )
				self:DrawShadow( false )
			end
		end
	end
	
	scripted_ents.Register( ENT, "metal_lune" )
end

if true then
	local ENT = {}
	ENT.Base = "base_anim"
	ENT.PrintName = "metalciel"
	ENT.Spawnable = false
	...
	function ENT:Initialize()
		...
		timer.Simple(config.tmpFall,function()
			if IsValid(self) then
				local metal = ents.Create( "metal_big" ) 
	...
	if CLIENT then
		function ENT:Draw()
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "ciel_metal_effect", ef )
				self:DrawShadow( false )
			end
		end
	end
	scripted_ents.Register( ENT, "ciel_metal" )
end
...
if true then
	...
	effects.Register( EFFECT, "metal_big_effect" )
end
...
if true then
	...
	effects.Register( EFFECT, "ciel_metal_effect" )
end