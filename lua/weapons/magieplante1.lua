...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local plante1 = ents.Create( "plante1" )
			plante1:SetPos( pos )
			plante1:SetAngles( ang ) 
			plante1:SetOwner( own )
			plante1:Spawn() 
			plante1:Activate() 
			own:DeleteOnRemove( plante1 )
			plante1:GetPhysicsObject():SetVelocity( dir )
			plante1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "plante1"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[15]_flower_slash", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "plante1" )
end