...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		if SERVER then
			...
			local ent8 = ents.Create("explosion4")
			ent8:SetPos(pos)
			ent8.Owner = self.Owner
			ent8:Spawn()
			SafeRemoveEntityDelayed(ent8,1)
...
if true then
	...
	function ENT:Initialize()
		...
		self:EmitSound( "ambient/explosions/explode_2.wav" , 100, 100,0.4)

		local idx = "eeeexxplossion"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "gas_explosion", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	if CLIENT then
		function ENT:Draw()
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "gas_explosion", ef )
				self:DrawShadow( false )
			end
		end
	end
	scripted_ents.Register( ENT, "explosion4" )
end