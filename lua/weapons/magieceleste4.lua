...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		if SERVER then
			...
			local entceleste4 = ents.Create("celeste4")
			entceleste4:SetPos(pos)
			entceleste4.Owner = self.Owner
			entceleste4:Spawn()
			if self.Owner:Health()-config.delVie<=0 then
				self.Owner:Kill()
			end
			SafeRemoveEntityDelayed(entceleste4,3)

			local own = self.Owner
		
			timer.Create("dmgmagieceleste4"..self.Owner:EntIndex(),config.interval,config.nb,function()
...
if true then
	...
	function ENT:Initialize()
		...
		local idx = "ccccccellesttteeee4"..self:EntIndex()
			timer.Create(idx,0.5,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "marcha_skill", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "celeste4" )
end