...
function SWEP:PrimaryAttack() 
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
        ...
        timer.Create("feuEffect/" .. math.random(0, 100000) * 10, .1, 10, function()
            ParticleEffect("[10]_rengoku_start", plyPos + ply:GetForward() * 100 * coef, Angle(0, 0, 0))
            if SERVER then
                local dmgVectors = {}
