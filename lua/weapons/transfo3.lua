...
SWEP.PrintName = "Transformation 3"
...
SWEP.TransformationModel = "models/echo/ark/boa_pm_bigger.mdl"
SWEP.TransformationSpeedBoost = 2
SWEP.TransformationJumpBoost = 1.5
SWEP.WeaponToGive = "transfo3arme"
...
hook.Add("PlayerShouldTakeDamage", "PreventFallDamageAfterTransformation3", function(player, attacker)
    if IsValid(player) and player:IsPlayer() then
        local weapon = player:GetActiveWeapon()
        if IsValid(weapon) and (weapon:GetClass() == "weapon_transformation1" or weapon:GetClass() == "weapon_transformation2" or weapon:GetClass() == "weapon_transformation3") and weapon.IsJumpingAfterTransformation then
            return false
        end
    end
end)

hook.Add("PlayerDeath", "ResetPlayerModelOnDeath3", function(player)
