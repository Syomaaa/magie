...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local foudre1 = ents.Create( "foudre1" )
			foudre1:SetPos( pos )
			foudre1:SetAngles( ang ) 
			foudre1:SetOwner( own )
			foudre1:Spawn() 
			foudre1:Activate() 
			own:DeleteOnRemove( foudre1 )
			foudre1:GetPhysicsObject():SetVelocity( dir )
			foudre1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			self:EmitSound( "ambient/energy/electric_loop.wav" , 100, 100,0.4)
			local phys = self:GetPhysicsObject()
			if phys:IsValid() then
				phys:Wake()
				phys:EnableGravity(false)
			end
			local idx = "foudre1"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "ornstein_tracer", 3, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "foudre1" )
end