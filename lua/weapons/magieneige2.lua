...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
			...
			local neige2 = ents.Create( "neige2" )
			neige2:SetOwner( own ) 
			neige2:SetPos( pos )
			neige2:SetAngles( Angle( 0, own:GetAngles().yaw, 0 ) ) 
			neige2:Spawn() 
			neige2:Activate()
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "neige2"..self:EntIndex()
			timer.Create(idx,0.2,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "snow_aura", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "neige2" )
end