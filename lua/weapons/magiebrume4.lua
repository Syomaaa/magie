...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		if SERVER then
			...
			local entbrume4 = ents.Create("brume4")
			entbrume4:SetPos(pos)
			entbrume4.Owner = self.Owner
			entbrume4:Spawn()
			SafeRemoveEntityDelayed(entbrume4,3)

			local own = self.Owner
		
			timer.Create("dmgmagiebrume4"..self.Owner:EntIndex(),config.interval,config.nb,function()
...
if true then
	...
	function ENT:Initialize()
		...
		local idx = "bruuuuuuuuuuuummmmmmmmmmmmmmmeeeeeeeeeeeee"..self:EntIndex()
	...
	if CLIENT then
		function ENT:Draw()
			if !self.XDEBZ_Effect then self.XDEBZ_Effect = true
				local ef = EffectData()
				ef:SetEntity( self )
				util.Effect( "[0]_fog_ambient", ef )
				self:DrawShadow( false )
			end
		end
	end
	scripted_ents.Register( ENT, "brume4" )
end