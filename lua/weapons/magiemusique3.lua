...
function SWEP:PrimaryAttack() 
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
        ...
        timer.Create("musiqueEffect/" .. math.random(0, 100000) * 10, .1, 10, function()
            ParticleEffect("[1]_sound_bomb_3", plyPos + ply:GetForward() * 100 * coef, Angle(0, 0, 0))
            if SERVER then
                local dmgVectors = {}
                ...
                for _, vector in ipairs(dmgVectors) do
                    for __, ent in ipairs(ents.FindInSphere(vector, 300)) do
                        if ent:IsPlayer() and ent != ply then
                            if ( ent != self.Owner and (ent:IsNPC() or ent:IsPlayer() or string.find(ent:GetClass(),"prop")) )then
