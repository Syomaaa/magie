...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local sable1 = ents.Create( "sable1" )
			sable1:SetPos( pos )
			sable1:SetAngles( ang ) 
			sable1:SetOwner( own )
			sable1:Spawn() 
			sable1:Activate() 
			own:DeleteOnRemove( sable1 )
			sable1:GetPhysicsObject():SetVelocity( dir )
			sable1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "sable1"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[6]_sand_dash_add_3", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "sable1" )
end