...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local ombre1 = ents.Create( "ombre1" )
			ombre1:SetPos( pos )
			ombre1:SetAngles( ang ) 
			ombre1:SetOwner( own )
			ombre1:Spawn() 
			ombre1:Activate() 
			own:DeleteOnRemove( ombre1 )
			ombre1:GetPhysicsObject():SetVelocity( dir )
			ombre1:SetPhysicsAttacker( own )
...
if true then
		...
		if SERVER then
			...
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "dskart_fw_charge", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "ombre1" )
end