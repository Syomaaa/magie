...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local eau1 = ents.Create( "eau1" )
			eau1:SetPos( pos )
			eau1:SetAngles( ang ) 
			eau1:SetOwner( own )
			eau1:Spawn() 
			eau1:Activate() 
			own:DeleteOnRemove( eau1 )
			eau1:GetPhysicsObject():SetVelocity( dir )
			eau1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			self:EmitSound( "ambient/water/wave2.wav" , 100, 100,0.4)
			local phys = self:GetPhysicsObject()
			if phys:IsValid() then
				phys:Wake()
				phys:EnableGravity(false)
			end
			local idx = "eau"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[9]_fast_dance", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "eau1" )
end