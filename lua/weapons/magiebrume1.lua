...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local brume1 = ents.Create( "brume1" )
			brume1:SetPos( pos )
			brume1:SetAngles( ang ) 
			brume1:SetOwner( own )
			brume1:Spawn() 
			brume1:Activate() 
			own:DeleteOnRemove( brume1 )
			brume1:GetPhysicsObject():SetVelocity( dir )
			brume1:SetPhysicsAttacker( own )
...
if true then
		...
		if SERVER then
			...
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[2]_fog_slash", 3, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "brume1" )
end