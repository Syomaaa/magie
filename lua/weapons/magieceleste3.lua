...
function SWEP:PrimaryAttack()
	...
	timer.Create("celestepar"..own:EntIndex(),0.01,1,function()
		if self:IsValid() then
			local p = ents.Create( "info_particle_system" ) 
			p:SetKeyValue( "effect_name", "[0]_starsky" )
	...
	timer.Create("celestecercle"..own:EntIndex(),0.2,1,function()
		if self:IsValid() then
			local circle = ents.Create( "celeste_circle" )
...
if true then
	...
	function ENT:Initialize()
		...
		par:SetKeyValue( "effect_name", "dragon_ice_breath" )
	...
	scripted_ents.Register( ENT, "celeste_circle" )
end