...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local typhon1 = ents.Create( "typhon1" )
			typhon1:SetPos( pos )
			typhon1:SetAngles( ang ) 
			typhon1:SetOwner( own )
			typhon1:Spawn() 
			typhon1:Activate() 
			own:DeleteOnRemove( typhon1 )
			typhon1:GetPhysicsObject():SetVelocity( dir )
			typhon1:SetPhysicsAttacker( own )
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			self:EmitSound( "ambient/water/wave2.wav" , 100, 100,0.2)
			local phys = self:GetPhysicsObject()
			if phys:IsValid() then
				phys:Wake()
				phys:EnableGravity(false)
			end
			local idx = "typhon1"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[9]_water_circle", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "typhon1" )
end