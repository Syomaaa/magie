...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local file1 = ents.Create( "file1" )
			file1:SetPos( pos )
			file1:SetAngles( ang ) 
			file1:SetOwner( own )
			file1:Spawn() 
			file1:Activate() 
			own:DeleteOnRemove( file1 )
			file1:GetPhysicsObject():SetVelocity( dir )
			file1:SetPhysicsAttacker( own )
			file1:EmitSound("physics/surfaces/sand_impact_bullet"..math.random(1,4)..".wav", 60, 160, 0.6)
...
if true then
		...
		if SERVER then
			...
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[13]_spider_slice", 1, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "file1" )
end