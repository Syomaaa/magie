...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local etoile2 = ents.Create( "etoile2" )
			etoile2:SetPos( pos )
			etoile2:SetAngles( ang ) 
			etoile2:SetOwner( own )
			etoile2:Spawn() 
			etoile2:Activate() 
			own:DeleteOnRemove( etoile2 )
			etoile2:GetPhysicsObject():SetVelocity( dir )
			etoile2:SetPhysicsAttacker( own )
			etoile2:EmitSound("physics/surfaces/sand_impact_bullet"..math.random(1,4)..".wav", 60, 160, 0.6)
...
function SWEP:SecondaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local etoile2fixe = ents.Create( "etoile2fixe" )
			etoile2fixe:SetPos( pos )
			etoile2fixe:SetAngles( ang ) 
			etoile2fixe:SetOwner( own )
			etoile2fixe:Spawn() 
			etoile2fixe:Activate() 
			own:DeleteOnRemove( etoile2fixe )
			etoile2fixe:GetPhysicsObject():SetVelocity( dir )
			etoile2fixe:SetPhysicsAttacker( own )
			etoile2fixe:EmitSound("physics/surfaces/sand_impact_bullet"..math.random(1,4)..".wav", 60, 160, 0.6)
...
if true then
		...
		if SERVER then
			...
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					ParticleEffectAttach( "[1]_light_flash", 1, self ,1)
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "etoile2" )
end
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			local idx = "etoile2fixe"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					ParticleEffectAttach( "[1]_light_flash", 1, self ,1)
				else
	...
	scripted_ents.Register( ENT, "etoile2fixe" )
end