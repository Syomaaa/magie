...
function SWEP:PrimaryAttack()
	...
	if self.CooldownDelay < CurTime() then if !SERVER then return end
		...
		if SERVER then
			...
			local explo1 = ents.Create( "explosion1" )
			explo1:SetPos( pos )
			explo1:SetAngles( ang ) 
			explo1:SetOwner( own )
			explo1:Spawn() 
			explo1:Activate() 
			own:DeleteOnRemove( explo1 )
			explo1:GetPhysicsObject():SetVelocity( dir )
			explo1:SetPhysicsAttacker( own )
...
function SWEP:SecondaryAttack()
	...
	if self.NextAction1 > CurTime() then return end
	if self.CooldownDelay1 < CurTime() then if !SERVER then return end

	    if SERVER then local boom = ents.Create( "env_explosion" )
	    boom:SetPos( self.Owner:GetPos( ) )
	    boom:Spawn()
	    boom:Fire( "explode", "", 0 )
	    self.Owner:SetVelocity(self.Owner:GetForward() * 400 + Vector(0,0,350))
	    self.CooldownDelay1 = CurTime() + self.Cooldown1
	    self.NextAction1 = CurTime() + self.ActionDelay1
	    else		
...
if true then
	...
	function ENT:Initialize()
		if SERVER then
			...
			self:EmitSound( "ambient/explosions/explode_9.wav" , 100, 100,0.4)
			local phys = self:GetPhysicsObject()
			if phys:IsValid() then
				phys:Wake()
				phys:EnableGravity(false)
			end
			local idx = "explo"..self:EntIndex()
			timer.Create(idx,0.02,0,function()
				if IsValid(self) then
					...
					ParticleEffectAttach( "[1]_sound_bomb_2", 3, self, 1 )
				else
					timer.Remove(idx)
				end
			end)
	...
	scripted_ents.Register( ENT, "explosion1" )
end