AddCSLuaFile()

ENT.Type = "anim"
ENT.Base = "base_gmodentity"
ENT.PrintName = "Gaz 2"
ENT.Author = "CeiLciuZ"
ENT.Spawnable = false
ENT.AdminOnly = false


function ENT:Initialize()
	ParticleEffectAttach("antlion_gib_02", 1, self, 1)

	if SERVER then
		self:SetModel( "models/props_phx/construct/metal_dome360.mdl" )
		self:SetMaterial("models/props_combine/com_shield001a.vtf")
		self:PhysicsInit(SOLID_VPHYSICS) 
		self:SetCollisionGroup( COLLISION_GROUP_DEBRIS_TRIGGER )
		self:DrawShadow(false)
		self:SetSkin(1) 
	
		self:SetMoveType(MOVETYPE_VPHYSICS)  
		self:SetModelScale(0) 

		timer.Simple(1, function() if not self:IsValid() then return end
			self:Remove() 
		end)
	end
end

function ENT:Draw()
    self:DrawModel()
end

local function PoisonDmg(ent, num, attacker)

    local valent = ent:EntIndex()

    if !timer.Exists("DMGPOISON"..tostring(valent)) then
        timer.Create("DMGPOISON"..tostring(valent), 2, num, function()
            if IsValid(ent) then
                ent:TakeDamage(math.random(10, 10), attacker, DMG_POISON)   
            else
                timer.Remove("DMGPOISON"..tostring(valent))
            end
        end)
    end
end

function ENT:Think()
	if SERVER then
		for k,v in pairs(ents.FindInSphere( self:GetPos(), 300)) do
            local own = self.Owner
            if IsValid(v) then
                if IsValid(v) and v != own and (v:IsPlayer() or v:IsNPC() or type( v ) == "NextBot") then
                    v:TakeDamage(50, self:GetOwner(), DMG_POISON)   
                    PoisonDmg(v, 3, self:GetOwner())
                                        self:Remove()
				end
			end
		end
	end
end